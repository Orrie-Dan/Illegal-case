const CFG = {
  PORTAL: "https://gh.space.gov.rw/portal/",
...
const F = {
  ...
  inspectingDate: "inspecting_date",
  description: "case_description",
  actionTaken: "action_taken",
  globalid: "globalid",
  verificationStatus: "committeeverification_status",
  sector: "sector",
  cell: "cell",
  village: "village",
  committeeAction: "committee_action",
  field_suggested_actions: "field_suggested_actions",
...
let pieInst = null,
  ...
  sectorInst = null,
  timeMode = "week";
let sectorMode = "sector",
  sectorVerFilter = "",
  sectorActFilter = "";
let timeVerFilter = "verified",
  timeActFilter = "";
...
const fd = (ts) =>
  ts
    ? new Date(ts).toLocaleDateString("en-RW", {
        year: "numeric",
        month: "short",
        day: "numeric",
      })
...
function normVer(v) {
  ...
  if (s.includes("review") || s.includes("progress") || s === "2")
    return "under_review";
...
function isNC(c) {
  const v = String(c.rawVS || "").toLowerCase(),
    lbl = (S.statMap[c.rawVS] || "").toLowerCase();
  return (
    v.includes("illegal") ||
    lbl.includes("illegal") ||
    v === "2" ||
    lbl.includes("non")
  );
}
function pillClass(a) {
  return (
    {
      Fine: "pill-fine",
      Demolish: "pill-demolish",
      "New Permit": "pill-newpermit",
      "Renew Permit": "pill-renewpermit",
    }[a] || "pill-pending"
  );
}
...
require([
  ...
  view
    ...
    .then(() => {
      fl.fields.forEach((f) => {
        if (f.name === F.source && f.domain?.codedValues)
          f.domain.codedValues.forEach((cv) => {
            S.srcMap[cv.code] = cv.name;
          });
        if (f.name === F.visitStatus && f.domain?.codedValues)
          f.domain.codedValues.forEach((cv) => {
            S.statMap[cv.code] = cv.name;
          });
      });
      if (!Object.keys(S.statMap).length)
        S.statMap = {
          0: "Not Visited",
          1: "Visited Legal",
          2: "Visited Illegal",
      ...
      Object.entries(S.srcMap).forEach(([code, label]) => {
    ...
    .then(async ([apiCases, arcResults]) => {
      ...
      const arcFeatures =
        arcResults && arcResults.features ? arcResults.features : [];
      const geoByOid = Object.create(null);
      const geoByGlobalId = Object.create(null);
      arcFeatures.forEach((f) => {
        ...
        if (oid != null && Number.isFinite(Number(oid)))
          geoByOid[Number(oid)] = f.geometry;
        if (gid)
          geoByGlobalId[String(gid).replace(/[{}]/g, "").toLowerCase()] =
            f.geometry;
      });
      ...
      if (!Array.isArray(apiCases) || apiCases.length === 0) {
        apiCases = arcFeatures.map((f) => {
          ...
          return {
            ...
            upi: a[F.upi] || "",
            visit_status:
              S.statMap[a[F.visitStatus]] || String(a[F.visitStatus] || ""),
      ...
      const codeForLabel = {};
      Object.entries(S.statMap).forEach(([code, label]) => {
        codeForLabel[String(label).toLowerCase()] = code;
        codeForLabel[String(label)] = code;
      });
      ...
      S.cases = apiCases.map((row) => {
        ...
        const rawVS =
          codeForLabel[String(row.visit_status || "").toLowerCase()] ||
          codeForLabel[row.visit_status] ||
          row.visit_status;
        const geo =
          oid != null && geoByOid[oid] !== undefined
            ? geoByOid[oid]
            : (row.globalid
                ? geoByGlobalId[
                    String(row.globalid).replace(/[{}]/g, "").toLowerCase()
                  ]
                : null) || null;
        return {
          id,
          ...
          srcLabel:
            row.source_data != null
              ? S.srcMap[row.source_data] || String(row.source_data)
              : "",
          rawVS,
          ...
          attachments: normAttachments(
            row.attachments ||
              row.case_attachments ||
              row.attachment_urls ||
              row.files,
          ),
          fineAmt:
            row.fine_amount != null && !isNaN(Number(row.fine_amount))
      ...
      const sectors = [
        ...new Set(S.cases.map((c) => c.sector).filter(Boolean)),
      ].sort();
      ...
      const cells = [
        ...new Set(S.cases.map((c) => c.cell).filter(Boolean)),
      ].sort();
      ...
      if (actSel) {
        ["Fine", "Demolish", "New Permit", "Renew Permit"].forEach((a) => {
          const o = document.createElement("option");
          o.value = a;
          o.textContent = a;
    ...
    .catch((err) => {
      console.error(err);
      var errEl = document.getElementById("cases-tbody");
      if (errEl)
        errEl.innerHTML =
          '<tr><td colspan="5" style="padding:16px;text-align:center;font-size:12px;color:var(--text-muted)">Connection Error — check portal access</td></tr>';
  ...
  window.APP = {
    applyFilters() {
      ...
      S.filtered = S.cases.filter((c) => {
        ...
        if (src !== "") {
          const srcMatch =
            String(c.source) === String(src) ||
            c.srcLabel === src ||
            (S.srcMap[src] && c.srcLabel === S.srcMap[src]);
        ...
        if (
          search &&
          ![c.id, c.upi, c.sector, c.cell, c.village, c.description]
            .join(" ")
            .toLowerCase()
            .includes(search)
        )
    ...
    updateSourceCards() {
      ...
      if (satelliteEl)
        satelliteEl.textContent = String(byLabel["Satellite"] || 0);
      ...
      document.querySelectorAll(".source-card").forEach((card) => {
        card.classList.toggle(
          "active",
          (card.getAttribute("data-source") || "") === activeLabel,
        );
      });
    },
    setSourceFilter(labelOrCode) {
      ...
      if (!sel) return;
      if (labelOrCode === "" || labelOrCode == null) {
        sel.value = "";
        this.applyFilters();
        return;
      }
      const code =
        typeof labelOrCode === "string" && isNaN(Number(labelOrCode))
          ? Object.entries(S.srcMap).find(
              ([c, l]) =>
                String(l).toLowerCase() === String(labelOrCode).toLowerCase(),
            )?.[0]
          : labelOrCode;
    ...
    renderList() {
      ...
      if (!total) {
        tbody.innerHTML =
          '<tr><td colspan="5" style="padding:16px;text-align:center;font-size:12px;color:var(--text-muted)">No cases match current filters</td></tr>';
        return;
      }
      const sortBy =
        (document.getElementById("table-sort-by") || {}).value || "date_asc";
      ...
      const sorted = [...S.filtered].sort((a, b) => {
        ...
        let cmp = 0;
        if (sortBy === "date_asc" || sortBy === "date") {
          cmp = da - db;
          ...
          cmp = idA.localeCompare(idB) || da - db;
        } else if (sortBy === "id_desc") {
          cmp = idB.localeCompare(idA) || da - db;
        } else if (sortBy === "sector") {
          cmp = secA.localeCompare(secB) || da - db;
        } else if (sortBy === "upi") {
          cmp = upiA.localeCompare(upiB) || da - db;
        } else if (sortBy === "visitstatus") {
          cmp = vsA.localeCompare(vsB) || da - db;
        } else {
          cmp = da - db;
        }
        return cmp;
    ...
    openView(id) {
      ...
      document.getElementById("view-cmt-btn").onclick = () => {
    ...
    openCmt(id) {
      ...
      ["c", "nc", "rv"].forEach((d) => {
        document.getElementById("dlbl-" + d).className = "dec-label";
        document.getElementById("dradio-" + d).className = "dec-radio";
      });
      ...
      ["fine", "demo", "new", "renew"].forEach((a) => {
        const el = document.getElementById("chk-" + a);
        if (el) {
          el.checked = false;
        }
        document.getElementById("ach-" + a).classList.remove("on");
      });
    ...
    pickDec(d) {
      ...
      ["c", "nc", "rv"].forEach((k) => {
        document.getElementById("dlbl-" + k).className =
          "dec-label" + (k === d ? " " + selMap[k] : "");
        const r = document.getElementById("dradio-" + k);
      ...
      const notesGroup = document.getElementById("notes-group");
      if (notesGroup) {
        notesGroup.style.display = d === "rv" ? "block" : "none";
      }
    },
    toggleAct(key, el) {
    ...
    async saveDecision() {
      ...
      if (S.dec === "nc") {
        ...
        acts = chks
          .filter((a) => document.getElementById("chk-" + a).checked)
          .map(
            (a) =>
              ({
                fine: "Fine",
        ...
        if (acts.includes("Fine")) {
          ...
          if (!amt || isNaN(amt) || amt <= 0) {
            this.toast(
              "Fine amount is missing or invalid for this case (RWF)",
              "error",
            );
            return;
          }
        ...
        const selVer = document.getElementById("cmt-ver")?.value;
        if (!selVer) {
      ...
      if (caseIdentifier == null) {
        this.toast(
          "Case OBJECTID is missing or invalid — cannot save decision",
          "error",
        );
        return;
      }
      ...
      const url =
        baseUrl +
        "/decision-enforcement/from-case/" +
        encodeURIComponent(caseIdentifier);
      const payload = {
        ...
        comment:
          committeeVerificationStatus === "Review" ? comment || null : null,
      };
    ...
    renderPie() {
      const nc = S.filtered.length
        ? S.filtered
        : S.cases.filter((c) => isNC(c));
      let fi = 0,
        dm = 0,
        np = 0,
        rp = 0,
      ...
      const data = [fi, dm, np, rp, none],
        ...
        colors = ["#3b82f6", "#ef4444", "#f59e0b", "#22c55e", "#9ba2b8"];
      const ctx = document.getElementById("pieChart").getContext("2d");
      if (pieInst) {
        pieInst.data.datasets[0].data = data;
      ...
      const options = {
        ...
        scales: {
          x: {
            ...
            title: {
              display: true,
              text: sectorMode === "cell" ? "Cell" : "Sector",
            },
            grid: { display: false },
          },
          y: {
    ...
    renderLine() {
      ...
      const seriesCases = base.filter((c) => {
        if (!c.inspDate) return false;
        if (timeVerFilter === "verified" && c.verNorm !== "verified")
          return false;
        if (timeActFilter) {
      ...
      let labels, data;
      if (timeMode === "week") {
        const ws = Array.from({ length: 12 }, (_, i) => i).reverse();
        labels = ws.map((i) => (i === 0 ? "This week" : i + "w ago"));
        data = ws.map((i) => buckets["W" + i] || 0);
      } else {
        labels = [...monthNames];
        data = labels.map((m) => buckets[m] || 0);
      }
      const xTitle =
        timeMode === "week" ? "Weeks (last 12)" : "Months (Jan–Dec)";
          ...
          datasets: [
            {
              ...
              data,
              backgroundColor: "rgba(15,113,115,0.18)",
              borderColor: "#0F7173",
              borderWidth: 2,
              borderRadius: 5,
              hoverBackgroundColor: "rgba(15,113,115,0.32)",
            },
          ],
        },
        options: {
    ...
    chips(f) {
      ...
      Object.entries(f).forEach(([k, v]) => {
        ...
        s.className = "chip";
        s.innerHTML = `${k}: ${esc(v)} <span class="chip-x" onclick="APP.clearChip('${k}')">&times;</span>`;
        el.appendChild(s);
      });
    },
    clearChip(k) {
      ...
      if (m[k]) document.getElementById(m[k]).value = "";
      this.applyFilters();
    },
    resetDashboard() {
      ["filter-source", "filter-verification", "filter-sector"].forEach(
  ...
  document.querySelectorAll(".modal-overlay").forEach((o) => {
    o.addEventListener("click", (e) => {
      if (e.target === o) APP.closeModal(o.id);
    });
  });

  const locModeSel = document.getElementById("chart-loc-mode");
  if (locModeSel) {
    locModeSel.addEventListener("change", (e) => {
      sectorMode = e.target.value || "sector";
      APP.renderSector();
    });
  }
  const locVerSel = document.getElementById("chart-loc-ver");
  if (locVerSel) {
    locVerSel.addEventListener("change", (e) => {
      sectorVerFilter = e.target.value || "";
      APP.renderSector();
    });
  }
  const locActSel = document.getElementById("chart-loc-act");
  if (locActSel) {
    locActSel.addEventListener("change", (e) => {
      sectorActFilter = e.target.value || "";
      APP.renderSector();
    });
  }
  ...
  if (timeVerSel) {
    timeVerSel.addEventListener("change", (e) => {
      timeVerFilter = e.target.value || "";
      APP.renderLine();
    });
  }
  const timeActSel = document.getElementById("chart-time-act");
  if (timeActSel) {
    timeActSel.addEventListener("change", (e) => {
      timeActFilter = e.target.value || "";
      APP.renderLine();
    });
  }
