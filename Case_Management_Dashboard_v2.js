    ...
    .then(async ([apiCases, arcResults]) => {
      ...
      arcFeatures.forEach((f) => {
        ...
        if (oid != null && Number.isFinite(Number(oid)))
          geoByOid.set(Number(oid), f.geometry);
        if (gid)
          geoByGlobalId.set(
            String(gid).replace(/[{}]/g, "").toLowerCase(),
            f.geometry,
          );
      });
  ...
  window.APP = {
    ...
    pickDec(d) {
      ...
      document.getElementById("action-panel").style.display =
        d === "nc" ? "block" : "none";
      document.getElementById("auto-note").style.display =
        d === "nc" ? "flex" : "none";
      document.getElementById("ver-group").style.display =
        d === "nc" ? "none" : "block";
    },
    toggleAct(key, el) {
